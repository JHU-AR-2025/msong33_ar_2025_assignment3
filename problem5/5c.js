var createScene = function () {
    ...
    */
    ...
    // Point light 2 (emits from a position, with distance falloff)
    uniform vec3 uPointLightColor2; // Linear RGB intensity/color of the point light
    uniform vec3 uPointLightPos2;   // World-space position of the point light
    ...
    vec3 shadePointLight(vec3 lColor, vec3 lPos, vec3 N, vec3 V, vec3 kd, vec3 ks, float shin)
    {
        ...
        */
        ...
        // YOUR CODE HERE
        vec3 lDir = lPos - vPosW;
        vec3 L = normalize(lDir);
        float diff = max(dot(N, L), 0.0);
        vec3 R = reflect(-L, N);
        float spec = pow(max(dot(R, V), 0.0), shin);
        vec3 I = kd*diff + ks*spec;
        return lColor*I; // Dummy return value. Please change this
    }
    ...
    void main()
    {
        ...
        // Add point light 2 contribution
        color += shadePointLight(
            uPointLightColor2,
            uPointLightPos2,
            N, V,
    ...
    var shaderMat = new BABYLON.ShaderMaterial("custom", scene, "custom", {
        ...
        uniforms: [
            ...
            "uPointLightColor2", "uPointLightPos2"
        ]
    });
    ...
    // Point Light 2
    shaderMat.setColor3("uPointLightColor2", new BABYLON.Color3(0.77, 1, 0.7));
    shaderMat.setVector3("uPointLightPos2", new BABYLON.Vector3(-2.5, -4, -2.0));
